<%@ attribute name="includeCancelButton" required="true" %>
<%@ attribute name="cancelButtonUrl" required="false" %>
<%@ attribute name="cancelButtonTabIndex" required="false" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://displaytag.sf.net" prefix="display"%>
<display:setProperty name="paging.banner.no_items_found">
    <div class="actions">
        <c:if test="${includeCancelButton}">
            <a href="${cancelButtonUrl}" class="cancel" tabindex="${cancelButtonTabIndex}"><fmt:message key="cancel" /></a>
        </c:if>
...
<display:setProperty name="paging.banner.onepage">
        <span class="pagelinks">{0}</span></span></div>
</display:setProperty>