...
<protExpress:tabPane paneTitleKey="experiment.tabs.experimentRuns" submittingPaneMessageKey="deleting" ignoreSuccessMessage="true">
    ...
    <c:url value="/experiment/management/load.action" var="sortUrl">
        <c:param name="initialTab" value="experimentRun" />
    </c:url>
    <display:table class="searchresults" cellspacing="0" defaultsort="1" excludedParams="initialTab" requestURI="${sortUrl}" list="${experiment.experimentRuns}" pagesize="5" sort="list" id="row">
        ...
        <display:column titleKey="actions" sortable="false">
            <c:if test="${row.auditInfo.creator == currentUser.loginName}">
                ...
                <s:a href="${deleteUrl}" onclick="showSubmittingText();" theme="ajax" targets="boxinner"><fmt:message key="delete" /></s:a>
            </c:if>
        </display:column>
    </display:table>
    <div class="actions">
